...
public class DateUtil
{
    ...
    //获取指定日期的前几天(-)或后几天 例：2019-12-01  2 --> 20191203
    ...
    /**
     * 获取指定日期的首日 	7留 	14留  30留  	60留  90留
     * @param checkDate
     * @return
     */
    public static List<String>  getDate(String checkDate)
    {
        List<String> dateList = new ArrayList<>();
        dateList.add(checkDate);
        dateList.add(getDateByNum(checkDate,6));
        dateList.add(getDateByNum(checkDate,13));
        dateList.add(getDateByNum(checkDate,29));
        dateList.add(getDateByNum(checkDate,59));
        dateList.add(getDateByNum(checkDate,89));
     ...
     *      @return String
     */
    public static String  secToTime(Integer times){
        String result = "0";
        if (times>0) {
            int hour = (int)Math.floor(times / 3600);
            int minute =  (int)Math.floor((times-3600 * hour)/60);
            int second =  (int)Math.floor(((times-3600 * hour) - 60 * minute) % 60);
     ...
     * @return
     */
    public static String longToDate(long ltime)
    {
        String strDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(ltime * 1000));
